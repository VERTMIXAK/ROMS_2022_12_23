...
# elif defined UV_QDRAG
      DO j=JstrT,JendT          ! based on Chezy coefficient (g/c^2)
        DO i=IstrT,IendT
!  Was 1.8 then 1.6, then 1.2, 0.8
          cff=0.4_r8*GRID(ng)%h(i,j)*LOG(GRID(ng)%h(i,j))
          rdrag2(i,j)=g/(cff*cff)
        END DO
      END DO
# endif
#elif defined BARROW || BARROWtelescope || BARROWB || MCKR
# if defined UV_LDRAG
      cff =  3.0d-04
      h0 = 1000.
      DO j=JstrT,JendT
...
# elif defined UV_QDRAG
      DO j=JstrT,JendT          ! based on Chezy coefficient (g/c^2)
        DO i=IstrT,IendT
!  Try less drag than Arctic?
          cff=1.0_r8*GRID(ng)%h(i,j)*LOG(GRID(ng)%h(i,j))
          rdrag2(i,j)=g/(cff*cff)
        END DO
      END DO
# endif
