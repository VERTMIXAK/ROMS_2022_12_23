...
#    echo "start date stamp is: " $date	
	...
	myYear=`grep $dstart ~/arch/addl_Scripts/dayConverter.txt | cut -d '-' -f1`
	myDay=`grep $dstart ~/arch/addl_Scripts/dayConverter.txt | tr -s ' ' | tr ' ' ',' | cut -d ',' -f4`
	date=$myYear'_'$myDay
    echo "start date stamp is: " $date
...
lbcStart=`grep -n '   LBC(isFsur)' $runInputFile | cut -d ":" -f1`
lbcEnd=`echo " 8 + $lbcStart " |bc` 
...
mkdir $exptName
mkdir $exptName/Apps
cp -r $Appsdir/* $exptName/Apps
cp runROMS.bash $exptName/Apps
cp Apps/varinfo.dat $exptName/Apps
...
#    echo '#SBATCH --partition=t2small'              													>>$batName 
    ...
    echo "mpirun -np $nProc  oceanM ./Apps/ocean_$exptname.in > log"   									>>$batName 
