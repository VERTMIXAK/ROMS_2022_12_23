...
% SCSA footprint - this is to size the TPXO stuff
lon0 = 114;lon1 = 126;
lat0 = 18;  lat1 = 23;
...
% game the times to refer to 1/1/1992    
    times = times - times(1) + 33602;
...
subplot(2,2,1);  
myMean = nanmean(model.amp_zeta(:));
imagesc(model.lon,model.lat,model.amp_zeta);axis xy;rect;hold on
contour(model.lon,model.lat,model.amp_zeta,[0:.1:.5],'LineColor','Black','Showtext','on')
title( ['ROMS - ',myTide,' amp (mean=',num2str( myMean ),')']);colorbar;
caxis([0 .55])

subplot(2,2,3);
myMean=nanmean(tpxo.amp_zeta_BB(:));
imagesc(model.lon,model.lat,tpxo.amp_zeta_BB);axis xy;rect;hold on
...
%caxis([288,292]);
rect;hold on
contour(model.lon,model.lat,tpxo.pha_zeta_BB ,[0:30:150],'LineColor','Black','Showtext','on')
